...
fn main() {
    ...
    loop {
        ...
        let pixels = pixels;    // no longer mutable

        let (img_img, img_future) = ImmutableImage::from_iter(
            pixels.iter().cloned(),
        ...
        img_future.then_signal_fence_and_flush().unwrap().wait(None).unwrap();
        ...
        Arc::new(PersistentDescriptorSet::start(pipeline.clone(), 0)
            ...
            .add_sampled_image(img_img.clone(), sampler.clone())
            .unwrap()
            .build().unwrap()
        );
